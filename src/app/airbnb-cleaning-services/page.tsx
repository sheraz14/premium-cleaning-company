  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative w-full h-auto min-h-[600px] flex overflow-hidden pt-12 md:pt-20 pb-0 md:py-0">
        <div className="absolute inset-0 w-full h-full -z-10 bg-indigo-100 hidden md:block">
          <img
        ...
        <div
          ...
          style={{
            background: "linear-gradient(to right, #eef2ff 20%, #f5f3ff 37%, rgba(238,242,255,0.05) 50%, transparent 70%)"
          }}
        />
        <div className="w-full max-w-full md:max-w-xl flex flex-col items-center w-full px-2 md:px-4 py-8 md:py-12 z-10 -mt-20 md:-mt-30">
          <div className="mb-6 flex flex-col items-center w-full">
            <span className="inline-flex items-center gap-2 text-indigo-700 font-semibold text-sm md:text-base mb-2">
              <ShieldCheck className="h-5 w-5 text-indigo-500" />
              100% Satisfaction Guarantee
            </span>
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold bg-gradient-to-r from-indigo-700 via-violet-700 to-fuchsia-600 bg-clip-text text-transparent mb-2 text-center drop-shadow-lg w-full">
              Get Your AirBnB Cleaning Quote
            </h1>
            <p className="text-base sm:text-lg text-muted-foreground mb-2 text-center font-medium w-full">
              Specialized AirBnB cleaning for hosts who want 5-star reviews and quick turnovers. <span className="font-bold bg-gradient-to-r from-indigo-700 via-violet-700 to-fuchsia-600 bg-clip-text text-transparent">Dust Drifters</span> ensures your rental is spotless, welcoming, and ready for every guest!
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full bg-white/70 backdrop-blur-md rounded-2xl p-4 md:p-8 shadow-2xl border border-white/60">
            ...
            {/* Cleaning type is fixed to AirBnB Cleaning for this page */}
            ...
            <button type="submit" className="mt-2 w-full bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-400 text-white font-bold shadow-lg hover:scale-105 hover:shadow-xl transition-transform duration-200 rounded-lg py-2 flex items-center justify-center" disabled={submitting}>
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-2 w-full">
            ...
            <span className="text-sm text-muted-foreground font-medium mt-1">Trusted by 100+ happy hosts</span>
          </div>
        </div>
        <div className="absolute left-0 right-0 bottom-0 h-24 md:h-32 z-20 pointer-events-none" style={{background: "linear-gradient(to bottom, rgba(255,255,255,0) 0%, #eef2ff 90%)"}} />
      </section>
      ...
      {/* Why Invest in Professional AirBnB Cleaning? Section */}
      <section className="w-full py-16 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-indigo-100/90 backdrop-blur-sm text-indigo-800 shadow-md mb-4 justify-center">
              <Star className="h-5 w-5 mr-2 text-indigo-500" />
              <span className="text-base font-medium">Why Invest?</span>
            </div>
            <h3 className="text-3xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-indigo-700 via-violet-700 to-fuchsia-600 bg-clip-text text-transparent">
              Why Invest in Professional AirBnB Cleaning?
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="flex flex-col items-center text-center p-6 rounded-xl hover:bg-indigo-50/60 transition-all duration-300 hover:shadow-md">
              <div className="bg-indigo-100 p-4 rounded-full mb-4">
                <Sparkles className="h-8 w-8 text-indigo-600" />
              </div>
              <h4 className="text-xl font-semibold text-indigo-800 mb-3">5-Star Guest Experience</h4>
              <p className="text-indigo-900/80">Impress every guest with a spotless, welcoming space that earns you top reviews and repeat bookings.</p>
            </div>
            <div className="flex flex-col items-center text-center p-6 rounded-xl hover:bg-indigo-50/60 transition-all duration-300 hover:shadow-md">
              <div className="bg-indigo-100 p-4 rounded-full mb-4">
                <ShieldCheck className="h-8 w-8 text-indigo-600" />
              </div>
              <h4 className="text-xl font-semibold text-indigo-800 mb-3">Health & Safety</h4>
              <p className="text-indigo-900/80">Our cleaning protocols reduce germs and allergens, helping you meet guest expectations for cleanliness and safety.</p>
            </div>
            <div className="flex flex-col items-center text-center p-6 rounded-xl hover:bg-indigo-50/60 transition-all duration-300 hover:shadow-md">
              <div className="bg-indigo-100 p-4 rounded-full mb-4">
                <Star className="h-8 w-8 text-indigo-600" />
              </div>
              <h4 className="text-xl font-semibold text-indigo-800 mb-3">Stand Out from Competition</h4>
              <p className="text-indigo-900/80">A professionally cleaned rental stands out in listings and attracts more bookings.</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col items-center text-center p-6 rounded-xl hover:bg-indigo-50/60 transition-all duration-300 hover:shadow-md">
              ...
              <h4 className="text-xl font-semibold text-indigo-800 mb-3">Focus on Hosting</h4>
              <p className="text-indigo-900/80">Let us handle turnovers so you can focus on your guests and growing your rental business.</p>
            </div>
            <div className="flex flex-col items-center text-center p-6 rounded-xl hover:bg-indigo-50/60 transition-all duration-300 hover:shadow-md">
              <div className="bg-indigo-100 p-4 rounded-full mb-4">
                <CalendarCheck2 className="h-8 w-8 text-indigo-600" />
              </div>
              <h4 className="text-xl font-semibold text-indigo-800 mb-3">Fast Turnarounds</h4>
              <p className="text-indigo-900/80">We work quickly and reliably to prepare your rental for the next guest, even on tight schedules.</p>
            </div>
          </div>
        </div>
      </section>
