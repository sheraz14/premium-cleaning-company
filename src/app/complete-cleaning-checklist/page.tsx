...
function SectionCard({ title, standard, deep, move, expanded, onClick, idx, image }: any) {
  let alt = title + ' photo';
  return (
    <div
      className={
        ...
        <div className="flex flex-col gap-8">
          <div>
            ...
            <ul className="space-y-2">
              {standard.map((item: string, i: number) => (
                <li key={i} className="flex items-start text-gray-700">
                  <CheckCircle2 className="h-5 w-5 text-purple-600 mr-2 mt-0.5 flex-shrink-0" /> {/* Assuming primary is purple */}
                  <span>{item}</span>
                </li>
              ))}
            </ul>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        ...
        {/* New Heading Section */}
        <div className="relative p-6 sm:p-10 rounded-xl shadow-2xl mb-12 sm:mb-16 text-center overflow-hidden">
          {/* Overlapping vector image grid background (from CTA) */}
          <div className="absolute inset-0 z-0 pointer-events-none">
            {Array.from({ length: 2 }).map((_, row) =>
              Array.from({ length: 5 }).map((_, col) => (
                <img
                  key={`header-bgimg-${row}-${col}`}
          ...
          {/* Overlay for smooth dark background */}
          <div className="absolute inset-0 z-10" style={{ background: 'rgba(30,41,59,0.75)' }} />
          <div className="relative z-20">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 drop-shadow-lg">
              Complete Cleaning Checklist
            </h1>
            <p className="text-lg text-slate-200 max-w-3xl mx-auto">
              Our comprehensive cleaning checklist covers all essential areas of your home. Deep Cleaning and Move In/Out extras are clearly marked. All services are for <span className="font-semibold text-indigo-300">interior only</span>.
            </p>
          </div>
        </div>

        {/* Flex container for Checklist and Extras sections */}
        <div className="flex flex-col lg:flex-row lg:gap-8 mb-16 lg:items-start">
          ...
          <div className="lg:w-1/3 mt-8 lg:mt-0">
            <div className="bg-yellow-50/90 backdrop-blur-sm border-l-4 border-yellow-400 p-8 rounded-xl text-center shadow-xl h-full">
              ...
              <p className="mb-4 text-yellow-800"> {/* Darkened text for better readability on slightly transparent bg */}
              ...
              <ul className="space-y-2 text-yellow-800 text-left inline-block"> {/* Darkened text */}
                {extras.map((item, i) => (
