...
export function ServicesHeroSection() {
  ...
  // Feature badges for under the form
  const features = [
    ...
    <section className="relative w-full h-auto min-h-[600px] flex overflow-hidden pt-12 md:pt-20 pb-0 md:py-0">
      ...
      <div className="w-full max-w-full md:max-w-xl flex flex-col items-center w-full px-2 md:px-4 py-8 md:py-12 z-10 -mt-20 md:-mt-30">
        ...
        <div className="mb-6 flex flex-col items-center w-full">
          <span className="inline-flex items-center gap-2 text-green-600 font-semibold text-sm md:text-base mb-2">
            <ShieldCheck className="h-5 w-5 text-green-500" />
            100% Satisfaction Guarantee
          </span>
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold bg-gradient-to-r from-indigo-700 via-purple-700 to-emerald-600 bg-clip-text text-transparent mb-2 text-center drop-shadow-lg w-full">
            Get Your Free Cleaning Quote
          </h1>
          <p className="text-base sm:text-lg text-muted-foreground mb-2 text-center font-medium w-full">Book today and experience the <span className="font-bold bg-gradient-to-r from-indigo-700 via-purple-700 to-emerald-600 bg-clip-text text-transparent">Dust Drifters</span> difference!</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full bg-white/70 backdrop-blur-md rounded-2xl p-4 md:p-8 shadow-2xl border border-white/60">
          ...
          <select name="cleaning" value={form.cleaning} onChange={handleChange} required className="rounded-lg border border-slate-200 px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary transition text-gray-700 bg-white/80 w-full">
          ...
          <Button type="submit" size="lg" className="mt-2 w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-emerald-400 text-white font-bold shadow-lg hover:scale-105 hover:shadow-xl transition-transform duration-200" disabled={submitting}>
